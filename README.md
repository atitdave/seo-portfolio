# Hi, I'm Atit Dave 👋

### SEO, AEO & GEO Strategist · Kitchener, ON

I help brands get found, ranked, and read — in classic search **and** in AI-driven answers. 8+ years driving organic growth across enterprise eCommerce, SaaS, B2B, healthcare, and industrial brands, now extended into Answer Engine Optimization (AEO) and Generative Engine Optimization (GEO).

📫 [atit.dave365@gmail.com](mailto:atit.dave365@gmail.com) &nbsp;·&nbsp; 💼 [LinkedIn](https://www.linkedin.com/in/atitdave365/)

---

## 📈 Career highlights

| Result | Context |
|---|---|
| **+168%** organic traffic | in a single 6-month engagement |
| **+100%** website traffic | from a full redesign built on segmentation + competitor research |
| **+62% / +46%** | traffic growth / goal-conversion growth from an SEO strategy |
| **1,000+** keywords | ranked #1 on Google |
| **+43% / −22%** | organic traffic up, bounce rate down via authority building |
| **35** projects | managed concurrently across SEO & PPC |

---

## 🧠 What I do

**Traditional SEO**
`Technical audits` · `Site architecture & crawlability` · `Keyword & intent mapping` · `On-page optimization` · `CRO` · `SEM` · `Penalty recovery` · `Authority building`

**AEO — Answer Engine Optimization**
`Featured snippets` · `Structured data & entity signals` · `Google AI Overviews` · `Answer-focused content formatting`

**GEO — Generative Engine Optimization**
`ChatGPT & Perplexity discoverability` · `Bing Copilot & Gemini` · `Topical authority for generative search` · `Content structuring for AI summarization`

---

## 🛠️ Tools & platforms

**SEO & Analytics:** SEMrush · Ahrefs · Google Search Console · Screaming Frog · GA4 · Adobe Analytics · Looker Studio · HubSpot · Crazy Egg

**CMS:** WordPress · Shopify · Joomla

**AI Search:** Google AI Overviews · ChatGPT · Perplexity · Bing Copilot · Gemini · Claude

**Also:** basic PHP / HTML / CSS

---

## 💼 Experience

**SEO Strategist** — Treefrog Inc., Newmarket, ON · *Mar 2023 – Present*
Own SEO and eCommerce merchandising strategy across Shopify & WordPress for SMB-to-enterprise DTC clients. Optimize content for featured snippets, AI Overviews, and generative search using structured data and entity signals; track KPIs via GA4 & Adobe Analytics; lead off-page authority building.

**SEO Manager** — CA Digital (Studio45) · *2022 – 2023*
Built and executed combined AEO/SEO action plans (content, technical, link building, keyword). Created a Looker Studio reporting system for Operations & Marketing stakeholders; mentored junior SEOs.

**SEO Quality Lead** — Studio45 IT Services, India · *2013 – 2021*
Managed 35 concurrent SEO/PPC projects for AU & US clients. Secured 1,000+ #1 Google rankings, grew organic traffic 43%, and led a redesign that doubled site traffic. Recovered a Google-penalized site.

---

## 🎓 Education & certifications

**Bachelor of Commerce** — Gujarat University, India
## 📜 Certifications & Licenses

| Certification | Issuing Organization | Issued | Expires | Credential ID | Skills | Credential Link |
|---|---|---|---|---|---|---|
| Google Ads Search Certification | Google | Jun 2026 | Jun 2027 | — | PPC, Google Ads | [Show credential](https://skillshop.credential.net/fe182311-a166-455b-ab5f-260593349760) |
| AI and Digital Marketing Trends | LinkedIn | May 2026 | — | — | AI for Marketing | [Show credential](https://www.linkedin.com/learning/certificates/a0f9ce5b3cf1dee2089df752159ac1e51080283f80d9c669ef681d1ada47cc06/) |
| Mastering SEO: How to Develop a Superior SEO Strategy | LinkedIn | May 2026 | — | — | Search Engine Optimization (SEO), Marketing Strategy | [Show credential](https://www.linkedin.com/learning/certificates/5d00bb19610e452fa49fe4a47c1d627e6f5bf21bffc6e80f78332b48447b8ba7/) |
| Marketing Tools: Digital Marketing Tools and Services | LinkedIn | May 2026 | — | — | Digital Marketing | [Show credential](https://www.linkedin.com/learning/certificates/a253ba886eaef8a0140b79cd95031721c17369513c9dbfbe5da801a2177f9cc2/) |
| SEO II | HubSpot Academy | Feb 2026 | Mar 2028 | bcd3dc88fb9e4260b69452d2476d73aa | Search Strategy, Link Building Campaigns | [Show credential](https://app-na3.hubspot.com/academy/achievements/hl17q60h/en/1/atit-dave/seo-ii) |
| Google Analytics Certification | Google | Feb 2026 | Feb 2027 | — | Search Strategy, Google Search Console, +2 skills | [Show credential](https://skillshop.credential.net/c5ab8125-4cf7-4e19-90d1-797987880636) |
